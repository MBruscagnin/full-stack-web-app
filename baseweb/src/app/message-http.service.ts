...
export class MessageHttpService {
  ...
  get_privMessages(b): Observable<Message[]> {
    return this.http.get<Message[]>( this.us.url + '/messages/' +b, this.create_options( {limit: '10', skip: '0'}  ) ).pipe(
        tap( (data) => console.log(JSON.stringify(data))) ,
        catchError( this.handleError )
      );
  }
...
/*
  post_privMessage( m: Message ): Observable<Message> {
    console.log('Posting ' + JSON.stringify(m) );
    return this.http.post<Message>( this.us.url + '/messages/' +b, m,  this.create_options() ).pipe(
      catchError(this.handleError)
    );
  }
*/
